...
export type SignUpSuccess = ActionWithPayload<types.SIGN_UP_SUCCESS, { user: User, additionalDetails: AdditionalInformation}>;
...
export type SignOutFailed = ActionWithPayload<types.SIGN_OUT_FAILED, Error>;
...
//  *extending UserData to have id 
//  because in user saga , getting the `id` from userSnapshot.id
export const signInSuccess = withMatcher((user: UserData & { id : string }): SignInSuccess => createAction(types.SIGN_IN_SUCCESS, user));
...
export const signUpSuccess = withMatcher((user: User, additionalDetails: AdditionalInformation): SignUpSuccess => createAction(types.SIGN_UP_SUCCESS, { user, additionalDetails }));
...
export const signOutFailed = withMatcher((error: Error): SignOutFailed => createAction(types.SIGN_OUT_FAILED, error));